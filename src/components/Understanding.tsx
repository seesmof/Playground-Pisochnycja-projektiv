...
const buttons: ButtonProps[] = [
  { href: "#mono", title: "What is a Monorepo?" },
  { href: "#poly", title: "The Polyrepo Tax" },
  { href: "#ai", title: "Monorepos Amplify AI Agents" },
  { href: "#synthetic", title: "Synthetic Monorepos" },
  { href: "#best", title: "What a Best-in-Class Tool Provides" },
];

export default function UndersatingSection() {
  return (
    <div className="flex flex-col gap-4 p-3 px-4 mt-32">
      ...
      <div className="grid grid-cols-2 gap-3 mt-24">
        {buttons.map((button, index) => (
          <Button key={index} href={button.href} title={button.title} />
        ))}
      </div>
    </div>
  );
